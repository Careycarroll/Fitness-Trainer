...
import { toFitNotesCSV, dateCollisions } from '../storage/fitnotes-export.js';
...
export function mount(root, defs) {
  ...
  root.querySelector('#export-plan').addEventListener('click', () => {
    ...
    try {
      ...
      // #25 requires warning about destination-date collisions. The export ADDS
      // rows and cannot tell FitNotes to replace, so a clash means two lots of
      // work on one day.
      const clashes = dateCollisions(out.dates, state.importedSets);
      if (clashes.length) {
        const proceed = window.confirm(
          clashes.length + ' of these dates already hold imported history:\n\n'
          + clashes.join(', ')
          + '\n\nExporting ADDS rows rather than replacing them, so those days '
          + 'will hold both. Continue?'
        );
        if (!proceed) {
          storageNote = 'Export cancelled.';
          paintStatus();
          return;
        }
      }
