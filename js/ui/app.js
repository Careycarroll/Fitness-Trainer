...
// Generated by tools/build_instructions.py. The SLUGS only, ~2 KB — the prose is
// loaded lazily on first disclosure open (see loadInstructions below).
//
// renderSetGroup is synchronous and cannot await a dynamic import, so "does this
// exercise have instructions" has to be answerable without the text. Rendering a
// disclosure on all 307 rows and apologising on open was the alternative, and a
// control that promises something it cannot deliver is the defect class this
// repo keeps finding rather than one to add.
import instructionSlugs from '../data/instruction-slugs.json' with { type: 'json' };

const HAS_INSTRUCTIONS = new Set(instructionSlugs.slugs ?? []);

/**
 * The instruction prose, fetched once and cached — #63.
 *
 * The PROMISE is cached rather than the result, so two cards opened in quick
 * succession share one fetch instead of racing. Vite emits this as its own
 * chunk, and workbox precaches it because it matches the `json` glob, so it is
 * available offline (ADR-001) without sitting on the cold-start path.
 */
let instructionPromise = null;

function loadInstructions() {
  instructionPromise ??= import('../data/instructions.json', { with: { type: 'json' } })
    .then((m) => m.default.instructions ?? {});
  return instructionPromise;
}

/**
 * Fill one disclosure, once.
 *
 * `dataset.filled` guards a single element against a double fill. On failure it
 * is CLEARED, so a second click retries rather than leaving the card reading
 * "Loading" forever.
 */
async function fillInstructions(summary) {
  const details = summary.closest('details');
  if (!details || details.dataset.filled) return;
  details.dataset.filled = '1';

  const list = details.querySelector('.howto-steps');
  if (!list) return;

  try {
    const all = await loadInstructions();
    const entry = all[summary.dataset.howto];
    list.innerHTML = entry?.steps?.length
      ? entry.steps.map((s) => `<li>${esc(s)}</li>`).join('')
      : '<li class="note">No instructions recorded for this exercise.</li>';
  } catch (err) {
    details.dataset.filled = '';
    list.innerHTML = `<li class="note">Could not load instructions: ${esc(err.message)}</li>`;
  }
}
...
export function mount(root, defs) {
  ...
  out.addEventListener('click', (e) => {
    ...
    // #63. `toggle` does NOT bubble, so a disclosure cannot join this delegated
    // chain through its own event. The summary's CLICK does bubble and fires
    // before the open state flips, so it hangs off that. Nothing calls
    // preventDefault, so the details still opens natively.
    const howto = e.target.closest('[data-howto]');
    if (howto) {
      fillInstructions(howto);
      return;
    }
...
function renderSetGroup(group, w, s, b, g, domain, blockType, rounds) {
  ...
  // #63. Rendered ONLY where text exists — 99 of 307 rows today. A disclosure
  // on the other 208 would open onto an apology, and the review pass fills them
  // in over time rather than all at once.
  const howto = HAS_INSTRUCTIONS.has(group.exerciseId)
    ? `<details class="howto">
        <summary data-howto="${esc(group.exerciseId)}">How to perform</summary>
        <ol class="howto-steps"><li class="note">Loading…</li></ol>
       </details>`
    : '';
